...
banco, option_1, option_2, option_3, option_4 = banco.center(29, "."), option_1.center(29), option_2.center(29), option_3.center(29), option_4.center(29)
...
def escrever_extrato(transacao, valores: tuple, *, valor_sacar=0) -> None:
    ...
    dic = {"saque": valor_sacar, "data": data(), "saldo_anterior": round(saldo_anterior), "saldo_atual": round(saldo_atual)} if transacao == "saque" else {"deposito": int(saldo_atual-saldo_anterior), "data": data(), "saldo_anterior": round(saldo_anterior), "saldo_atual": round(saldo_atual)}
    extrato_texto = "saque de R${saque}.00 realizado no dia {data}, seu saldo foi de R${saldo_anterior}.00 para R${saldo_atual}.00".format(**dic) if transacao == "saque" else "deposito de R${deposito}.00 realizado no dia {data}, seu saldo foi de R${saldo_anterior}.00 para R${saldo_atual}.00".format(**dic)
...
def sair_do_programa_ou_voltar_ao_menu(mostrar_opcao__extrato=True):
    ...
    option_2 = "Digite [e] para abrir o extrato".center(29) if mostrar_opcao__extrato is True else "Digite [x] para fechar o programa".center(29)
    ...
    """.format(**{"pergunta": "Você deseja voltar para ao menu anterior ou fechar o programa?", "banco": banco, "option_1": option_1, "option_2": option_2, "option_3": option_3, "pontos": "."*29})
